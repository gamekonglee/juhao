...
import bc.juhao.com.listener.ISingleTouchViewListener;
...
public class SingleTouchView extends View {
	...
	private void init(){
		...
		if(controlDrawable == null){
			controlDrawable = getContext().getResources().getDrawable(R.drawable.design_transform);
		}
...
//		}
	...
	private void adjustLayout(int type){
		...
		if(type==2 ){
			if(mViewPaddingLeft != newPaddingLeft || mViewPaddingTop != newPaddingTop){
				...
				Log.e("520it", "l:"+newPaddingLeft+" t:"+newPaddingTop+" r:"+newPaddingLeft + actualWidth+" b:"+newPaddingTop + actualHeight);
			}else{
				layout(newPaddingLeft, newPaddingTop, newPaddingLeft + actualWidth, newPaddingTop + actualHeight);
				Log.e("520it", "l:"+newPaddingLeft+" t:"+newPaddingTop+" r:"+newPaddingLeft + actualWidth+" b:"+newPaddingTop + actualHeight);
			}
		}else if(type==1 && isScale){
			if(mViewPaddingLeft != newPaddingLeft || mViewPaddingTop != newPaddingTop){
				mViewPaddingLeft = newPaddingLeft;
				mViewPaddingTop = newPaddingTop;
	...
	protected void onDraw(Canvas canvas) {
		...
		Log.e("520it", "isScale:"+isScale);
		adjustLayout(1);
		Log.e("520it", "adjustLayout: 1");
		super.onDraw(canvas);
	...
	private ISingleTouchViewListener mListener;

	public void setListener(ISingleTouchViewListener listener) {
		mListener = listener;
	}

	public boolean onTouchEvent(MotionEvent event) {
		mListener.onSingleTouchChange(true);
		if(!isEditable){
...
//				// 修改中心点
				...
				adjustLayout(2);
				Log.e("520it", "adjustLayout: 2"+" mCurMovePointFx:"+mCurMovePointF.x+"  mCurMovePointF.y"+mCurMovePointF.y);
			}
...
//		this.mPreMovePointF = mCenterPoint;
//		this.mCurMovePointF = mCenterPoint;
		adjustLayout(3);
		Log.e("520it", "adjustLayout: 3");
	}
